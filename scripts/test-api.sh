...
# Test Posts API
echo ""
print_info "Testing Posts API..."
echo "======================"

# Create a post (if user exists)
if [[ ! -z "$USER_ID" ]]; then
    print_info "1. Creating a post..."
    ...
    # Get all posts
    print_info "2. Getting all posts..."
    POSTS_RESPONSE=$(curl -s "$API_BASE/posts")
    if [[ $? -eq 0 ]]; then
        print_success "Retrieved all posts"
        POST_COUNT=$(echo $POSTS_RESPONSE | grep -o '"id"' | wc -l)
        print_info "Total posts: $POST_COUNT"
    else
        print_error "Failed to get posts"
    fi

    # Get published posts
    print_info "3. Getting published posts..."
    PUBLISHED_RESPONSE=$(curl -s "$API_BASE/posts/published")
    if [[ $? -eq 0 ]]; then
        print_success "Retrieved published posts"
        PUBLISHED_COUNT=$(echo $PUBLISHED_RESPONSE | grep -o '"id"' | wc -l)
        print_info "Published posts: $PUBLISHED_COUNT"
    else
        print_error "Failed to get published posts"
    fi

    # Get posts by author
    print_info "4. Getting posts by author: $USER_ID"
    AUTHOR_POSTS_RESPONSE=$(curl -s "$API_BASE/posts/author/$USER_ID")
    if [[ $? -eq 0 ]]; then
        print_success "Retrieved posts by author"
        AUTHOR_POSTS_COUNT=$(echo $AUTHOR_POSTS_RESPONSE | grep -o '"id"' | wc -l)
        print_info "Posts by author: $AUTHOR_POSTS_COUNT"
    else
        print_error "Failed to get posts by author"
    fi

    # Get post by ID (if post was created)
    if [[ ! -z "$POST_ID" ]]; then
        print_info "5. Getting post by ID: $POST_ID"
        POST_BY_ID_RESPONSE=$(curl -s "$API_BASE/posts/$POST_ID")
        if [[ $? -eq 0 ]]; then
            print_success "Retrieved post by ID"
        else
            print_error "Failed to get post by ID"
        fi

        # Update post
        print_info "6. Updating post: $POST_ID"
        UPDATE_POST_RESPONSE=$(curl -s -X PATCH "$API_BASE/posts/$POST_ID" \
          -H "Content-Type: application/json" \
          -d '{
    ...
    print_warning "Skipping posts tests - no user ID available"
fi
